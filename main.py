...
def check_and_install_requirements():
    """
    Installation de secours (utile en local).
    Sur Render, privilégie requirements.txt (Build Command).
    """
    required_packages = {
...
if __name__ == "__main__":
    # Optionnel (utile surtout en local)
    ...
    async def check_and_update_files():
        """
        Vérifie autoupdateinfo.txt et propose des mises à jour de fichiers.
        En environnement non interactif (Render), on ne demande pas d'input.
        """
        try:
            # Essai GitHub
            try:
            ...
            with sqlite3.connect('db/settings.sqlite') as conn:
                ...
                if updates_needed:
                    ...
                    # Environnement non interactif (Render): on ne met pas à jour automatiquement
                    if os.getenv("NON_INTERACTIVE", "1") == "1":
                        print(Fore.YELLOW + "\nNON_INTERACTIVE=1 → Skipping file updates on this environment." + Style.RESET_ALL)
                        return False

                    # Sinon (local), on peut demander
                    ...
                    if response_input == 'y':
                        ...
                        for file_name, new_version in updates_needed:
                            if file_name.strip() != 'main.py':
                                ...
                                if file_response.status_code == 200:
                                    ...
                                    content_file = file_response.text.rstrip('\n')
                        ...
                        if main_py_updated:
                            ...
                            if main_response.status_code == 200:
                                content_main = main_response.text.rstrip('\n')
    ...
    # --- Token via variables d'environnement ---
    BOT_TOKEN = os.getenv("DISCORD_TOKEN") or os.getenv("TOKEN")
    if not BOT_TOKEN:
        # fallback local: bot_token.txt
        token_file = 'bot_token.txt'
        if os.path.exists(token_file):
            with open(token_file, 'r') as f:
                BOT_TOKEN = f.read().strip()
    if not BOT_TOKEN:
        print("❌ No bot token found. Set DISCORD_TOKEN in environment variables.")
        sys.exit(1)

    # --- Dossiers/DB locales ---
    os.makedirs('db', exist_ok=True)
    print(Fore.GREEN + "db folder ensured" + Style.RESET_ALL)
    ...
    async def load_cogs():
        ...
        # Optionnel : initialiser l'API de giftcodes
        try:
            from cogs.gift_operationsapi import GiftCodeAPI
            bot.gift_api = GiftCodeAPI(bot)
        except Exception as e:
            print(Fore.YELLOW + f"GiftCodeAPI init skipped: {e}" + Style.RESET_ALL)
